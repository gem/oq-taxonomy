...
if [ ! -f html/configuration.php ] ; then
    ...
    sudo "cat $HOME/oq-taxonomy/configuration.php.tmpl | sed \"s/\(^[ 	]\+public \$secret = '\)[^']\+\(';\)/\1${NEW_SALT}\2/g;\
              s/\(^[ 	]\+public \$smtphost = '\)[^']\+\(';\)/\1${HOST_SMTP}\2/g;\" | tee /var/www/html/configuration.php"
fi
